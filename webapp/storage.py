...
def _read_json(filename, default_val):
    # Check temp dir first, then local filename
    tmp_path = os.path.join(tempfile.gettempdir(), filename)
    target = tmp_path if os.path.exists(tmp_path) else filename
...
def _write_json(filename, data):
    ...
    # Fallback to temp directory on Vercel read-only filesystem
    tmp_path = os.path.join(tempfile.gettempdir(), filename)
    try:
...
def export_orders_csv(filepath="orders_export.csv"):
    ...
    try:
        ...
        filepath = os.path.join(tempfile.gettempdir(), "orders_export.csv")
        with open(filepath, "w", encoding="utf-8-sig", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["Order ID", "User ID", "Name", "Phone", "Address", "Items Count", "Total Price", "Status"])
            for o in orders:
...
def export_orders_txt(filepath="orders_export.txt"):
    ...
    try:
        ...
        out_file = os.path.join(tempfile.gettempdir(), "orders_export.txt")
        f = open(out_file, "w", encoding="utf-8")
